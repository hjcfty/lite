...
async function play(flag, id, flags) {
    ...
    if (data && data.result) {
        ...
        if (res.check_url) {
            ...
            if (res.vod_url && res.vod_url.includes('.m3u8')) {
                ...
                return JSON.stringify({ parse: 0, url: res.vod_url, header: play_headers });
            }
        }
    }
    return '{}';
...
async function proxy(params) {
    ...
    if (payload.indexOf('TS@') > -1) {
        ...
        const resp = await req(hlsSign(payload), { headers: play_headers });
...
function hlsSign(url) {
    ...
    return `${url}${separator}wsSecret=${wsSecret}&wsTime=${hexTime}`;
}
